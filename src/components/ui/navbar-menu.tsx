          ...
          {active === item && (
            <div className="absolute top-[calc(100%+1.2rem)] left-1/2 transform -translate-x-1/2 pt-4">
              <motion.div
                ...
                className="bg-[#27272a] backdrop-blur-sm rounded-2xl overflow-hidden border border-black-20 dark:border-white-20 shadow-xl"
              >
                <motion.div
  ...
  return (
    <a href={href} className="flex space-x-2">
      ...
      <div>
        ...
        <p className="text-sm max-w-40 text-neutral-300">{description}</p>
      </div>
    </a>
  );
};
...
export const HoveredLink = ({ children, ...rest }: HoveredLinkProps) => {
  return (
    <a
      {...rest}
      className="text-neutral-200 hover:text-gray-400 transition duration-300 ease"
    >
      {children}
    </a>
  );
